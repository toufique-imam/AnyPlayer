...
/*
    fun onExoPlayerModelChanged(
        trackSelector: DefaultTrackSelector,
        trackSelectionArray: TrackSelectionArray
    ) {
        trackSelector.let { playbackService ->
            //val mappedTrackInfo = it.currentMappedTrackInfo ?: return@let
            if (playbackService.getVideoTracksCount() <= 2) {
                binding.videoTracks.trackContainer.setGone()
                binding.tracksSeparator3.setGone()
            }
            ...
            val videoTrackId = playbackService.getSelectedTrackType(
                C.TRACK_TYPE_VIDEO,
                trackSelectionArray
            )
            playbackService.getVideoTracks().let { trackList ->
                val trackAdapter = TrackAdapter(
                    trackList,
                    if (videoTrackId == C.INDEX_UNSET)
                        null
                    else trackList.firstOrNull { trackInfo ->
                        trackInfo.id == videoTrackId
                    }
                )
                trackAdapter.setOnTrackSelectedListener { track ->
                    trackSelectionListener.invoke(track)
            ...
            val audioTrackId = playbackService.getSelectedTrackType(
                C.TRACK_TYPE_AUDIO,
                trackSelectionArray
            )
            playbackService.getAudioTracks().let { trackList ->
                val trackAdapter = TrackAdapter(
                    trackList,
                    if (audioTrackId == C.INDEX_UNSET)
                        null
                    else trackList.firstOrNull { trackInfo ->
                        trackInfo.id == audioTrackId
                    }
                )
                trackAdapter.setOnTrackSelectedListener { track ->
                    trackSelectionListener.invoke(track)
            ...
            val subtitleTrackId =
                playbackService.getSelectedTrackType(C.TRACK_TYPE_TEXT, trackSelectionArray)
            playbackService.getSubtitleTracks().let { trackList ->
                val trackAdapter = TrackAdapter(
                    trackList,
                    if (subtitleTrackId == C.INDEX_UNSET)
                        null
                    else trackList.firstOrNull { trackInfo ->
                        trackInfo.id == subtitleTrackId
                    }
                )
                trackAdapter.setOnTrackSelectedListener { track ->
                    trackSelectionListener.invoke(track)
            ...
            if (playbackService.getSubtitleTracksCount() <= 0) binding.subtitleTracks.emptyView.setVisible()
